...
import br.ufrj.ppgi.huffmanhadoop.mapreduce.EncoderConfiguration;
import br.ufrj.ppgi.huffmanhadoop.mapreduce.SymbolCountConfiguration;

public class Encoder {
	int[] frequency = new int[Defines.POWER_BITS_CODIFICATION];
	short symbols = 0;
	NodeArray nodeArray;
	Codification[] codification;

	public Encoder(String file_in, String file_out, String file_cb)
			throws Exception {
	...
	public void fileToFrequency(String arg0) throws IOException {
		...
		for(byte i = 1 ; i < status.length ; i++) {
			...
			while(f.available() > 0) {
				byte symbol = f.readByte();
				frequency[symbol & 0xFF] = f.readInt();
				symbols++;
			}
		}
